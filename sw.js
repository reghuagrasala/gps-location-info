const CACHE="my-location-info-v29";
...
const EXTERNAL_ASSETS=["https://cdn.jsdelivr.net/npm/three@0.179.1/build/three.min.js","https://cdn.jsdelivr.net/npm/globe.gl@2.46.2/dist/globe.gl.min.js","https://cdn.jsdelivr.net/npm/three-globe/example/img/earth-blue-marble.jpg","https://cdn.jsdelivr.net/npm/three-globe/example/img/earth-topology.png","https://cdn.jsdelivr.net/npm/three-globe/example/img/night-sky.png","https://api.open-meteo.com/v1/forecast"];
...
self.addEventListener("fetch",event=>{
 ...
 event.respondWith((async()=>{
  if(isAPI){
   ...
   catch{const isAppJS=url.pathname.endsWith("/js/app.js")||url.pathname.endsWith("/js/globe.js")||url.pathname.endsWith("/styles.css")||url.pathname.endsWith("/index.html");\n  if(isAppJS){try{const fresh=await fetch(event.request,{cache:"no-store"});const c=await caches.open(CACHE);c.put(event.request,fresh.clone()).catch(()=>{});return fresh}catch{const cached=await caches.match(event.request);if(cached)return cached;}}\n  const cached=await caches.match(event.request);if(cached)return cached;return new Response("Offline",{status:503})}
  }

...
function updateSun(s){try{if(!s.g.lights)return;const lights=s.g.lights(),dir=lights?.find(x=>x&&x.isDirectionalLight);if(!dir)return;const now=new Date(),h=now.getUTCHours()+now.getUTCMinutes()/60,subLon=((180-h*15+540)%360)-180,lat=23.44*Math.sin((2*Math.PI*(now.getUTCDate()-80))/365.2422),p=s.g.getCoords(lat,subLon,1.2);dir.position.set(p.x,p.y,p.z);dir.intensity=2.35}catch{}}

(function(){
  ...
  function save(pos){
    ...
    },timestamp:pos.timestamp||Date.now(),source:"GPS (Device)"};
    state.position=p;state.error=null;
    try{localStorage.setItem(KEY,JSON.stringify(p))}catch(e){}
    emit();
  }
  function start(){
    if(!navigator.geolocation){state.gpsAvailable=false;emit();return;}
    navigator.geolocation.watchPosition(save,
      err=>{state.error=err;emit()},
      {enableHighAccuracy:true,maximumAge:0,timeout:10000});
  }
  window.MyLocationGPS={
    getState:()=>({...state}),

...
function show(v){v=views[v]?v:"home";document.body.classList.toggle("detail-mode",v!=="home");$$(".view").forEach(x=>x.classList.remove("active"));$("#"+views[v]).classList.add("active");history.replaceState({v},"","#"+v);window.scrollTo(0,0);if(v==="position"){renderSaved();setTimeout(()=>resizeGlobe($("#positionGlobe")),50)}if(v==="weather")renderWeather();renderAll()}
...
async function enrichPlace(p){if(geoBusy||!navigator.onLine||!p||!Number.isFinite(p.lat)||!Number.isFinite(p.lon))return;const now=Date.now();if(lastGeo.lat!==null&&(now-lastGeo.time<60000||distanceMeters(lastGeo.lat,lastGeo.lon,p.lat,p.lon)<150))return;geoBusy=true;try{const r=await reverseGeocode(p.lat,p.lon);if(r?.ok){p.place=r.label||p.place;p.address=r.address||p.address;p.postcode=r.postcode||p.postcode;lastGeo={lat:p.lat,lon:p.lon,time:Date.now()};renderAll()}}catch{}finally{geoBusy=false}}
...
onGPS(p=>{gps=p;renderAll();enrichPlace(p)});window.addEventListener("online",()=>{renderAll();if(gps)enrichPlace(gps)});window.addEventListener("pageshow",()=>{resizeGlobe($("#globe"));resizeGlobe($("#positionGlobe"));if(gps)renderAll()});window.addEventListener("resize",()=>{resizeGlobe($("#globe"));resizeGlobe($("#positionGlobe"))});document.addEventListener("visibilitychange",()=>{if(!document.hidden)renderAll()});

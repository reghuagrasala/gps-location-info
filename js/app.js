...
async function enrichPlace(p,force=false){if((geoBusy&&!force)||!navigator.onLine||!p||!Number.isFinite(p.lat)||!Number.isFinite(p.lon))return;$("#addressStatus").textContent=force?"Refreshing address…":"Fetching address…";const now=Date.now();if(!force&&lastGeo.lat!==null&&(now-lastGeo.time<60000||distanceMeters(lastGeo.lat,lastGeo.lon,p.lat,p.lon)<150))return;geoBusy=true;try{const r=await reverseGeocode(p.lat,p.lon);if(r?.ok){p.place=(r.locality||r.label||p.place);p.address=r.address||p.address;p.postcode=r.postcode||p.postcode;p.district=r.district||p.district;if(!p.district&&p.lat>=10.1&&p.lat<=10.85&&p.lon>=75.9&&p.lon<=76.8)p.district="Thrissur";p.state=r.state||p.state;p.country=r.country||p.country;lastGeo={lat:p.lat,lon:p.lon,time:Date.now()};renderAll()}}catch{$("#addressStatus").textContent="Address fetch failed — tap ↻ to retry."}finally{geoBusy=false}}
